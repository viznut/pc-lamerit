...
unsigned int csf_read(song_t *csf, void * v_buffer, unsigned int bufsize)
{
        ...
        bufleft = max;
//        fprintf(stderr,"bufleft=%d\n",bufleft);

        if (csf->flags & SONG_ENDREACHED)
        {
//                fprintf(stderr,"SONG_ENDREACHED\n");
...
//                        fprintf(stderr,"read_note!\n");

                        if (!csf_read_note(csf)) {
//                                fprintf(stderr,"read_note rc 0! endreached\n");
        ...
        if (--csf->tick_count == 0) {
                ...
                if (--csf->row_count <= 0) {
                        ...
                        if (++csf->process_row >= csf->pattern_size[csf->current_pattern]) {
                                ...
                                {
//                                        fprintf(stderr,"can't increment order!\n");
                                        return 0;
                                }
                        } /* else [-- No --] */
...
int csf_read_note(song_t *csf)
{
        ...
        if (csf->flags & SONG_PAUSED) {
                ...
                {
//                        fprintf(stderr,"couldn't process tick!\n");
                        return 0;
                }
        }
